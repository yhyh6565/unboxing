...
export const generateResultsPDF = async (
  ...
  room.questions.forEach((q, i) => {
    ...
    if (qAnswers.length === 0) {
      aHtml = `<div style="color:${c.muted};font-size:13px;">답변이 없습니다</div>`;
    } else {
      qAnswers.forEach(a => {
        aHtml += `<div style="background:${c.bg};padding:12px;margin-bottom:10px;border-left:4px solid ${c.primary};">
          ...
          <div style="font-size:12px;color:${c.accent};font-weight:bold;">— ${a.author_nickname}</div>
        </div>`;
      });
    }
  ...
  const html = `
    <div style="text-align:center;padding:40px;margin-bottom:30px;border:4px solid ${c.accent};background:${c.card};">
      ...
      <div style="font-size:16px;color:${c.text};margin-bottom:20px;">UNBOX US - 최종 결과</div>
      ...
      <div style="color:${c.muted};font-size:14px;">
        참여자 ${participants.length}명 · 질문 ${room.questions.length}개<br/>
        생성일: ${new Date(room.created_at).toLocaleDateString('ko-KR')}
      </div>
    </div>
    <div style="margin-bottom:30px;">
      <div style="font-size:20px;font-weight:bold;color:${c.accent};margin-bottom:20px;padding-bottom:10px;border-bottom:2px solid ${c.accent};">📋 질문별 답변 모음</div>
      ${qHtml}
    </div>
    <div style="margin-bottom:30px;">
      <div style="font-size:20px;font-weight:bold;color:${c.accent};margin-bottom:20px;padding-bottom:10px;border-bottom:2px solid ${c.accent};">👥 참여자별 답변 요약</div>
      ${pHtml}
    </div>
    <div style="text-align:center;padding:20px;color:${c.muted};font-size:12px;margin-top:40px;border-top:2px solid ${c.accent};">
      Generated by Unbox Us 🎁
    </div>
  `;
